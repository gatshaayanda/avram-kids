  ...
  return (
    <main className="site">
      ...
      <section id="equipment" className="section"><div className="container">
        ...
        <div className="equipmentGrid">{equipment.map((item) => <article className="equipmentCard" key={item.id}><div className="equipmentTop"><span className="equipmentPill">FOR HIRE</span><strong>{item.price}</strong></div><div className="equipmentVisual" aria-hidden="true">{item.name.toLowerCase().includes("water") || item.name.toLowerCase().includes("slide") ? "💦" : "🏰"}</div><h3>{item.name}</h3><p>{item.detail}</p><div className="equipmentMeta"><span>{item.size}</span><span>{item.price}</span></div><Link href={`/book?equipment=${encodeURIComponent(item.name)}`} className="button buttonLight">Request this</Link></article>)}</div>
